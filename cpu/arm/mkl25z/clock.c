...
clock_init()
{
	...
	TPM0_MOD = 0xFFFF;				      						/* Set up modulo register use all 16-bits */
...
#ifdef SYSTICK_DEBUG	
	gpio_set_output(SYSTICK_DEBUG_GPIO, port_pin_to_mask(SYSTICK_DEBUG_PIN));
	port_conf_pin(SYSTICK_DEBUG_PORT, SYSTICK_DEBUG_PIN, (PORT_PCR_MUX_GPIO | PORT_PCR_ISF_MASK));
	gpio_clr_pin(SYSTICK_DEBUG_GPIO, port_pin_to_mask(SYSTICK_DEBUG_PIN));
#endif		
}
